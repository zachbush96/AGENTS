...
from assistants import get_invstment_research_assistant, get_options_investor_assistant  # type: ignore
...
def main() -> None:
    # Get LLM Model
    #local_model = (st.sidebar.selectbox("Select LLM", options=["gemma:2b","dolphin-llama3:latest","llama2" ]))
    #Groq model
    model = (
    ...
    if st.sidebar.button("Generate Options Strategy"):
        options_strategy = st.session_state.get("options_strategy")
        if not options_strategy:
            # Initialize the Options Investor Assistant
            options_investor_assistant = get_options_investor_assistant(model=st.session_state["model"])
            st.session_state["options_investor_assistant"] = options_investor_assistant
            #Read in the report_input
            report_input = st.session_state["final_report"] 
            # Generate the strategy
            strategy_input = f"Please generate an options strategy based on the following stock report:\n\n{report_input}"
            options_strategy = ""
    ...
    if st.sidebar.button("Generate Strategy Instructions"):
        ...
        if not strategy_instructions:
            ...
            options_investor_assistant = get_options_investor_assistant(model=st.session_state["model"])
            st.session_state["options_investor_assistant"] = options_investor_assistant
            #Read the options_strategy from the session state
            options_strategy = st.session_state["options_strategy"]
            # Generate the strategy instructions
            strategy_instructions_input = f"Please generate instructions for the following options strategy:\n\n{options_strategy}"
            strategy_instructions = ""
            for delta in options_investor_assistant.run(strategy_instructions_input):
                strategy_instructions += delta
            st.session_state["strategy_instructions"] = strategy_instructions
        # Display the generated strategy instructions
        with st.container():
